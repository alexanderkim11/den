...
pub async fn execute(handle: tauri::AppHandle, mut command: Vec<String>) -> (bool, String) {
    ...
    let bin = if platform == "windows" {"leo-x86_64-pc-windows-msvc.exe"} else if platform == "macos" { if arch() == "aarch64" {"leo-aarch64-apple-darwin"} else {"leo-x86_64-apple-darwin"} } else {"leo-x86_64-unknown-linux-gnu"};
    let exec_bin = format!(
        "{}{}{}",
